...
class VehiculosModel extends Model
{
    ...
    protected $allowedFields = ['n_orden', 'id_marca', 'placa', 'linea', 'modelo', 'color', 'kms', 'n_combustible', 'estado', 'fecha_entrada', 'usuario_crea']; /* relacion de campos de la tabla */
    ...
    {
        $this->select("vehiculos.id_vehiculo as id, n_orden, propietarios.id_tercero as cliente, placa, id_marca, modelo, color, kms, param_detalle.id_param_det as combustible, vw_param_det.id_param_det as estado, fecha_entrada");
        $this->join('param_detalle', 'param_detalle.id_param_det = vehiculos.n_combustible', 'left');
        $this->join('vw_param_det', 'vw_param_det.id_param_det = vehiculos.estado', 'left');
        $this->join('propietarios', 'propietarios.id_vehiculo = vehiculos.id_vehiculo', 'left');
